...
public class InterlinkController {
    ...
    public void updatedFeed(HttpServletResponse response, @PathVariable String date) throws IOException {
        ...
        Iterator<Content> content = executor.updatedSince(Publisher.C4_PMLSD, from);
		outputter.output(adapter.fromContent(FEED_ID+date, Publisher.C4_PMLSD, from, to, content), response.getOutputStream(), false, from);
    }
    ...
    public void bootstrapFeed(HttpServletResponse response) throws IOException {
    	...
    	Iterator<Content> content = executor.updatedSince(Publisher.C4_PMLSD, from);
    	outputter.output(adapter.fromContent(FEED_ID + "bootstrap", Publisher.C4_PMLSD, from, to, content), response.getOutputStream(), true, from);
    }
}
