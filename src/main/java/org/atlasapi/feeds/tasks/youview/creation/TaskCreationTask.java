...
//            progress = progress.reduce(processOnDemand(onDemand.getKey(), onDemand.getValue()));
    ...
    private UpdateProgress processContent(Content content, Action action) {
        ...
        try {
            ...
            log.error("Failed to createDefault payload for content {}", content.getCanonicalUri(), e);
    ...
    private UpdateProgress processVersion(String versionCrid, ItemAndVersion versionHierarchy, Action action) {
        try {
            ...
            log.error(String.format(
                            "Failed to createDefault payload for content %s, version %s",
    ...
    private UpdateProgress processBroadcast(String broadcastImi, ItemBroadcastHierarchy broadcastHierarchy, Action action) {
        try {
            ...
            log.error(String.format(
                            "Failed to createDefault payload for content %s, version %s, broadcast %s",
...
//            if (action == Action.DELETE || shouldSave(HashType.ON_DEMAND, onDemandImi, p)) {
            ...
            log.error(String.format(
                            "Failed to createDefault payload for content %s, version %s, encoding %s, location %s",
