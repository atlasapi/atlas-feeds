...
public enum RadioPlayerFeedType {
	...
	public void compileFeedFor(DateTime date, RadioPlayerService service, KnownTypeQueryExecutor queryExecutor, OutputStream out) throws IOException {
		if(outputter != null) {
			...
			outputter.output(date, service, sorter.sortAndTransform(broadcastFilter.filter(queryExecutor.executeItemQuery(queryFor(date, serviceUri)), serviceUri, date), serviceUri, date), out);
		}
	}
}
