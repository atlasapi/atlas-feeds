...
public abstract class ChannelGenerator {
    ...
    abstract void setRelatedMaterial(Channel channel,
            ServiceInformationType serviceInformationType);

    protected Optional<Image> getBbcImageByAlias(
            Channel channel,
            Alias alias1,
            Alias alias2
    ) {
        return channel.getImages()
                .stream()
                .filter(image1 ->
                        image1.getAliases() != null &&
                                (image1.getAliases().contains(alias1)
                                        || image1.getAliases().contains(alias2)))
                .findFirst();
    }
    ...
    protected void setContentProperties(Image image,
            ExtendedRelatedMaterialType relatedMaterialType,
    ...
    protected void setPromotionalText(Channel channel,
